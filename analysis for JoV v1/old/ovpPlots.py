...
def ovp(exp,corr=False, trim=True, bins=8):
	...
	dm = dm.addField('land1_perc', dtype=float)
	dm = dm.calcPerc(fix1, 'land1_perc', nBin=bins)
	cmLat1 = dm.collapse(['land1_perc'], lat1)	
	cmLat2 = dm.collapse(['land1_perc'], lat2)
	cmX = cmFix1 = dm.collapse(['land1_perc'], fix1)
	cmFix2 = dm.collapse(['land1_perc'], fix2)
	cmProb = dm.collapse(['land1_perc'], prob)
	cmDur1 = dm.collapse(['land1_perc'], dur1)
	cmDur2 = dm.collapse(['land1_perc'], dur2)
	cmTotal = dm.collapse(['land1_perc'], total)
	cmRt = dm.collapse(['land1_perc'], rt)

	
	fig = plt.figure(figsize = (3,10))
	plt.subplots_adjust(left=.2, bottom=.15, hspace = 0.2)
		
	colList = [orange[1], blue[1]]
	lTitles = ["dur 1", "prob", "land 2", "total", "lat 1", "lat 2", "dur 2", "rt"]
	lTitles.reverse()
	...
	for cmY in [cmDur1, cmProb, cmFix2, cmTotal, cmLat1, cmLat2, cmDur2, cmRt]:#, cmLat1]:
		nPlot +=1
	...
	plt.savefig("ovp_%s_corr_%s.png" % (exp, corr))
