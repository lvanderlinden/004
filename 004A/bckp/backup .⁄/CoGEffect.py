...
if __name__ == "__main__":
	...
	# Uncorrected landing position:
	studies._004.analyses.anovas.oneFactor(dmAll, dv = "endXDegr", \
		factor = ["mask_side"], exControlMask = False, \
			exFiller = False, printStats = True, yLim = None,showFig = False)
	
	# Landing position corrected for CoG original bitmap:
	studies._004.analyses.anovas.oneFactor(dmAll, dv = "endXCorrDegr", \
		factor = ["mask_side"], exControlMask = False, \
			exFiller = False, printStats = True, yLim = None,showFig = False)

	# Landing position corrected for CoG with mask taken into account:
