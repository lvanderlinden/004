...
def ovp(nBin = 5, toHandle = False, onlyControl = True):
	...
	for exp in ["004A", "004B"]:
		...
		if toHandle:
			dm = getDM.getDM(exp = exp, onlyControl = onlyControl, \
				excludeFillers = True)
		...
		for binVar in lBinVar:
			...
			for dv in lVars:
				...
				colList = ["red", "blue"]
				for handle in on_dm.unique("handle_side"):
					
					handle_dm = on_dm.select("handle_side == '%s'" % handle)
				
					col = colList.pop()				
	
					# Make bins, only for the first dv (for the second dv,
					# the binned variable is the same and therefore 
					# already exists:
					binnedVar = "binnend%s" % binVar
					binned_dm = handle_dm.addField(binnedVar)
					binned_dm = binned_dm.calcPerc(binVar, binnedVar,\
						keys = ["file"], nBin = nBin)
					...
					plt.ylabel(dv)
					if plotCount == 15:
						plt.legend(on_dm.unique("handle_side"))
					
					
	plt.savefig("OVP_effects_ToHandle_split = %s onlyControl = %s.png" % \
		(toHandle, onlyControl))
